...
SELECT film_id, title, description, length, rental_rate FROM film WHERE title LIKE 'Di%';
-- 10. Write a query which will find the 10 cheapest movies.
SELECT film_id, title, rental_rate FROM film ORDER BY rental_rate, film_id ASC LIMIT 10;
-- 11. Not satisfied with the results. Write a query which will find the next 10 cheapest movies.
SELECT film_id, title, rental_rate FROM film ORDER BY rental_rate, film_id ASC LIMIT 10 OFFSET 10;
...
-- SELECT customer_id, amount, payment_date FROM payment ORDER BY customer_id ASC;, is this not the same thing?
...
FROM film
WHERE NOT EXISTS(SELECT 1 FROM inventory WHERE film.film_id = inventory.film_id)
